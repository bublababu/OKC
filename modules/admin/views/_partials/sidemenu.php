<ul class="sidebar-menu">
	...
	<?php //echo 'P'. $parent ; echo '<br>'; echo 'C' .$ctrler?>
		...
		<?php else: ?>
			...
			<?php
			$parent_active=0;
			   if($parent=='Reservations' && ( $ctrler=='lodge_reservations' || $ctrler=='reservations' )) { $parent_active=1; }
			   if($parent=='Reports' && ( $ctrler=='unfilled_harvest' || $ctrler=='harvest' || $ctrler=='reservation' || $ctrler=='guests' )) { $parent_active=1; }
			   
			    if($parent=='User Assessment' && ( $ctrler=='assessment'  )) { $parent_active=1; }
			?>
