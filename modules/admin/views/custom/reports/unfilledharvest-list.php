...
<div class="row">
	...
	<?php echo $form->messages(); ?>
        <div class="box box-primary">
            ...
            <div class="row p-3">
               <div class="col-sm-12 col-md-8 btn-group mb-4 member-field">
                    ...
                    <ul class="dropdown-menu text-capitalize ml-4">
                        <li><a class="member" href="/admin/reports/unfilled-harvest?member=all<?= $startDate_uri . $endDate_uri?>">All</a></li>                      
                        <?php  foreach ($members as $member) { ?>
                        <li><a class="member" href="/admin/reports/unfilled-harvest?member=<?=$member['user_id']?><?= $startDate_uri . $endDate_uri?>"><?=$member['first_name']?>&nbsp;<?=$member['last_name']?>&nbsp;(<?=$member['user_id']?>)</a></li>
                        <?php } ?>
                    </ul>
            ...
            <div class="col-md-2 pull-right pl-0 total-count">              
                <span class="pull-right rpt-teritory">Total: <?php echo $total_row ?></span>
            </div>
            <div class="col-md-12 p-3">
               <div class="custom-table-wrapper">
                  <table class="table table-bordered table-striped bycolor-table table-hover">
                      ...
                      <?php //print_r($reservations); exit; ?>
                          <tr>
                             
                             
                              <?php //$lease=array(); $lease = $controller->lease($reservations['reservation_type_id']);?>
                              <td><?=$reservations['leases_name']?></td>
                               <?php $lease_area=array(); $lease_area = $controller->lease_area($reservations['lease_area_id']);?>
                              <td><?=$lease_area[0]['name']?></td>
                              <td><?=$reservations['start_date']?> - <?=$reservations['end_date']?></td>
                              <td><?=$reservations['reservation_types_name']?></td>
                               <?php $user=array(); $user = $controller->user($reservations['user_id']);?>
                              <td><?=$user[0]['first_name']?>&nbsp;<?=$user[0]['last_name']?>&nbsp;(<?=$reservations['user_id']?>)</td>
                                <td>
                                  <ul class="list-unstyled">
                                     
                                      <?php $users=array(); $users = $controller->reservation_users($reservations['id']); ?>
                                      <?php if($reservations['use_spot']) { ?>
                                        <li><i class="fa fa-check color-green"></i> <?=$user[0]['first_name']?> <?=$user[0]['last_name']?></li>
                                        <?php } else { ?>
                                            <li><i class="fa fa-check color-green"></i> <?=$user[0]['first_name']?> <?=$user[0]['last_name']?>  (Member Not Attending)</li>
                                        <?php } ?>
...
$(function() {
   ...
   var start = new Date("<?php echo $startDate; ?>");
    ...
    $('#reportrange').daterangepicker(
        ...
        function(start, end) {
            ...
            var baseUrl = '/admin/reports/unfilled-harvest';
  ...
  $('.member').click(function(){
    ...
    $('.download_csv').click(function(){
            var baseUrl = '/admin/reports/unfilled-harvest/download_csv';
            var queryParameters = {}, queryString = location.search.substring(1), re = /([^&=]+)=([^&]*)/g, m;
            while (m = re.exec(queryString)) {
                queryParameters[decodeURIComponent(m[1])] = decodeURIComponent(m[2]);
            }            
